const ADMIN_API = 'https://vortexcollections-api.onrender.com';
...
protectAdminPage().then(allowed => {
  ...
  passwordForm?.addEventListener('submit', async event => {
    ...
    try {
      const response = await fetch(ADMIN_API + '/api/admin/users/' + encodeURIComponent(form.user_id.value) + '/password', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + localStorage.getItem('vortex_token') }, body: JSON.stringify({ password: form.new_password.value }) });

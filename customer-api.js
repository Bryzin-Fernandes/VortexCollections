...
const downloadEnv = { 'vortex-kitpvp': 'DOWNLOAD_KITPVP_URL', 'vortex-feast': 'DOWNLOAD_FEAST_URL', 'vortex-thepit': 'DOWNLOAD_THEPIT_URL', 'vortex-skywars': 'DOWNLOAD_SKYWARS_URL' };
...
function install({ app, pool, auth, bcrypt, vault, fingerprint, createLicenseKey, mercadoPago, provisionLicense }) {
  ...
  app.get('/api/me/licenses', auth, wrap(async (req, res) => {
    ...
    res.json(rows.map(row => {
      const active = row.status === 'active' && row.order_status === 'approved';
      let key = null;
      let key_error = false;
      ...
      return { id: row.id, product: row.product, slug: row.slug, status: active ? 'active' : row.status === 'active' ? 'suspended' : row.status,
        description: details[row.slug] || '', created_at: row.created_at, authorized_ips: row.authorized_ips,

...
function install({ app, pool, auth, mercadoPago, createLicenseKey, bcrypt, vault, fingerprint }) {
  ...
  const isPrimaryAdmin = async user => {
    if (!process.env.ADMIN_USER_ID || String(user.sub) !== String(process.env.ADMIN_USER_ID)) return false;
    const row = (await pool.query('SELECT role FROM users WHERE id=$1', [user.sub])).rows[0];
    return row?.role === 'admin';
  };
  const admin = [auth, wrap(async (req, res, next) => {
    if (!await isPrimaryAdmin(req.user)) return res.status(403).json({ error: 'Acesso restrito ao administrador principal. Confira o ADMIN_USER_ID e o role da sua conta.' });
    next();
  })];
  const staff = [auth, wrap(async (req, res, next) => {
    if (await isPrimaryAdmin(req.user)) return next();

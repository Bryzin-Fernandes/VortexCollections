(() => {
  ...
  const safeImage = value => { try { const url = new URL(String(value || ''), location.href); return url.protocol === 'https:' ? url.href : null; } catch (_) { return null; } };
  const levelForSpent = cents => { const spent = Math.max(0, Number(cents) || 0); let index = 0; levels.forEach((level,i) => { if(spent >= level.threshold_cents) index=i; }); const current=levels[index],next=levels[index+1]||null,span=next ? next.threshold_cents-current.threshold_cents : 0; return { ...current, next, to_next_cents: next ? Math.max(0,next.threshold_cents-spent) : 0, progress_percent: next ? Math.min(100,Math.max(0,Math.round(((spent-current.threshold_cents)/span)*1000)/10)) : 100 }; };
  const V = window.Vortex = {
    meta, levels, levelForSpent, apiBase:window.VORTEX_CONFIG.apiBase.replace(/\/$/,''),
  ...
  if(V.token()) V.userPromise=V.request('/api/me').then(user=>{const el=document.getElementById('header-account');if(el){el.replaceChildren(V.profileAvatar(user,32),V.el('span',user.minecraft_nick || user.name));}return user;}).catch(e=>{if(e.status===401){localStorage.removeItem('vortex_token');sessionStorage.removeItem('vortex_token');}return null;});else V.userPromise=Promise.resolve(null);
})();
